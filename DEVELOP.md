...
> **この 2 本目はまだ一度も本番で走っていない**（`v0.8.1` の時点では `samples/` が
> 無かった）。次にタグを打ったら、Release に 2 つ付いているかを見ること。
...
動かず、Release も作られない。`v0.7.0` がその状態で残っている
（ローカルにはあるが、リモートにもリリース一覧にも無い）——
遡って出すなら push、要らないなら `git tag -d v0.7.0`。
