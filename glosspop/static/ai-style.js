 ...
 *
 * `scopes` に並べたぶんだけ選べる（ビューアのサイドバーは 📁 だけ）。
 * `onChange(info)` は保存が通ったあとに呼ばれる —— 呼ぶ側が同じ応答で
 * 自分の画面を描き直せるように、`/api/ai/settings` と同じ形をそのまま渡す。
 *
 * `showPaths` を偽にすると**置き場所のパスを出さない**（狭いサイドバー用）。
 * 消すのはパスだけで、**「親フォルダのものが効いている」「辞書がないので置けない」は
 * 残す** —— どちらもパスではなく事実で、黙ると「全体に書いたのに効かない」
 * 「押せないのはなぜ」が見えなくなる。全文は ⚙ 側（`showPaths` が真）に出る。
 *
