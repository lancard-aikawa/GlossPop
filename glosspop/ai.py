...
_SCHEMA_HINT = """{
  ...
  "when": "その語自体が作中でいつのことか。事件・出来事だけ。分からなければ空文字",
  "tags": ["検索用タグを 0〜4 件"]
}"""

#: `when` の書き方をプロンプトに添える節。**捏造させないための節**なので、
#: 「分からなければ空」と「大体でよい」を両方書くこと —— 片方だけだと、
#: 正確な日付を作るか諦めるかの二択になり、AI は前者を選ぶ（実測）
_WHEN_RULES = """## 作中の時刻 (`when`)

- **書くのは事件・出来事・作品だけ。** その語自体が「いつのことか」が決まるものに限る。
- **人物には書かないこと。** 人は期間（生没）なので 1 点にできず、書くと
  「生まれか、没か、そのときの年齢か」が決まらなくなります。
- **先頭は西暦。** `1582-06-21` `1582` のように書き、うしろには元号でも
  作中の暦でも続けてよい（`1582-06-21 天正十年六月二日`）。並べ替えに使うのは
  先頭だけで、画面には書いたままが出ます。
- **大体しか分からないときは、分かるところまでで書く。**
  `16世紀` `1560年代` `約1560` `1560ごろ` と書けます。**正確な日付を作らないこと。**
- **元号や作中の暦しか書かれていないときは、西暦は「年」までにすること。**
  月日を西暦に直さないこと —— 和暦や太陰暦の月日は換算表が無いと西暦に直せず、
  数字を写しただけの日付は**数週間ずれます**（「天正十年六月二日」は西暦の
  6 月 2 日ではありません）。年だけ書いて、月日はうしろにそのまま残します:
  `1582 天正十年六月二日`。年が本文に書いてあるならそれを使ってよい
  （「天正十年（一五八二年）」のように併記されていることが多い）。
- **本文からも一般的な知識からも決まらないなら空文字。**
  それらしい日付を当てずっぽうで書かないこと —— 間違った時刻は、書かれていないより
  悪い（時系列と地図の並びが狂います）。"""
...
def build_prompt(
    ...
    *,
    ...
    style_block = build_style_block(
        ...
        "`term` `reading` `aliases` `category` `subcategory` `tags` `when`",
    )
    if style_block:
        parts += ["", style_block]
...
async def draft_entry(
    ...
    *,
    ...
    if source and not data.get("source"):
        data["source"] = source
    # **時刻の申告は丸呑みしない**（読みと同じ扱い）。西暦として読めるかは
    # 機械で確かめられるので、読めないものは落とす —— 通すと「書いたのに並ばない」
    # エントリができ、点検が毎回それを挙げることになる
    data["when"] = filter_when(data.get("when"))
    return EntryDraft.model_validate(data)


#: 漢数字の月（「六月」）。**これが続いているとき、頭の西暦の月日は信用しない。**
#: 太陰暦の月日は換算表が無いと西暦に直せないので、AI は数字を写すしかない
_KANJI_MONTH = re.compile(r"[一二三四五六七八九十]+\s*月")

#: 日本が太陽暦に変わった年（明治六年）。**これ以降は和暦と西暦の月日が一致する**
#: ので、揃っていても写し間違いではない
GREGORIAN_FROM = 1873


def filter_when(value: object) -> str:
    """AI が書いた作中の時刻を、**信じてよいところまで**に削る。

    2 段構え。**確かめようがあるものは確かめる**、という `filter_readings()` と
    同じ考え方（読みは「かなか」、こちらは「西暦として読めるか」）。

    1. **西暦として読めなければ落とす**（「元号だけ」「作中の暦だけ」）。
       大体の書き方は通る（`16世紀` `約1560`）ので、分かる範囲で書くぶんには落ちない
    2. **漢数字の月が続いていたら、頭は年だけに丸める。**
       `1570-06-28 元亀元年六月二十八日` → `1570 元亀元年六月二十八日`

    2 がいる理由は実測にある。**AI は和暦の月日をそのまま西暦の枠に写す** ——
    `六月二日` を `06-02` と書く（正しくは 1582-06-21 で、**3 週間ずれる**）。
    実測では 3 件中 3 件が同じ壊れ方で、**プロンプトで禁じても 3 件中 1 件しか
    直らなかった**。しかも 1 の検査は通ってしまう（形は正しいので）。
    ネタバレ抑止と同じで、**プロンプトの禁止文だけに頼らない**。

    **落とすのは月日だけで、年は残す。** 年は本文に併記されていることが多く
    （「天正十年（一五八二年）」）、粗い書き方は範囲の頭に置く規則がそのまま効く。
    **正しく換算されていたぶんも丸まる**が、**間違った日付は書かれていないより
    悪い**（読みと同じ判断）。

    **人が書いたものはここを通らない。** 手で正しい換算を書いたなら、その精度は
    そのまま残る —— 機械の申告にだけ枷をかける。
    """
    text = str(value or "").strip()
    if not text:
        return ""
    if whenfmt.sort_key(text) is None:
        return ""
    at = whenfmt.sort_key(text)
    year = at // 10 ** 10
    if year < GREGORIAN_FROM and _KANJI_MONTH.search(text):
        return whenfmt.year_only(text)
    return text
