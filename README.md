...
- **関係を図で見る** — 用語どうしの関係を書くと図になる。一覧と 5 つの図が
  同じタブに並ぶ（地図はその外）（→ [関係を図で見る](MANUAL.md#関係を図で見る)）
- **フォルダごとの辞書** — 小説の登場人物を全体の辞書に混ぜずに済む
  （→ [フォルダ辞書](MANUAL.md#フォルダ辞書ローカル辞書)）
- **ネタバレを防ぐ** — 初出の場面だけを AI に渡す。図でも判明位置つきの関係は伏せる
...
3. 好きな場所に展開し、**`GlossPop\glosspopw.exe`** を実行する（`w` 付きが
   ダブルクリック用。`glosspop.exe` はコマンドライン用で、こちらは黒い窓が出る）
...
exe だけ取り出しても動かない。`_internal\` と同じフォルダに置いたまま使うこと。
