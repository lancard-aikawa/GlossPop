...
- 索引は持たず、押すたびに読む。**300 文書 / 200 ヒットで打ち切り**、打ち切ったことと
  読めなかったファイルの件数は必ず画面に出る（黙って切ると「無かった」と区別が付かない）
...
- ローカルのカテゴリも**改名と削除はできる**（辞書ページの「カテゴリ管理」で `📁` が
  付いている行）。マスターが無いので、**並び順・説明・用語 0 件のカテゴリは持てない**
...
- **どこに書いたかは必ず stderr に出る。** 祖先の辞書が使われることがあるので、
  黙って別の場所に書かない
- カテゴリマスターはグローバル辞書のもの。ローカルに登録してもマスターには載らない
  （ビューアからの登録と同じ）
...
**取り込みは置き換え。** いまの辞書は zip の中身に入れ替わり、**zip に無い用語は消える**。
別の PC の辞書と混ぜることはできない（同じカテゴリの同じ用語が両側にあったときに
どちらを採るか決められないため）。
