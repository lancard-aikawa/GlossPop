...
書かせる本数）。20 本なら 5 分前後になるので、待ち時間は **探す本数**（10 / 20 / 40）で
調節する。実行中は経過秒が出て、**中止** でいつでも止められる。
...
⚙ の **AI（下書き・抽出・関係）** で、下書き・抽出・関係の下書きに使う AI を選べる。
**ここでの変更は次の呼び出しから効く**（再起動は要らない）。
...
`GlossPop-0.5.0` として展開する。展開したらそちらの `glosspop.exe` を起動すればよい。
