...
- **年表（作中の時刻順）は、相関図の[時系列](#見せ方は-6-つある)とは別のものです。**
...
- **時系列** — 上から順に「いつ」が見える見せ方。**この見せ方だけ「いつ」が
  ...
  **軸が 2 つあり、見せ方の右の選択で切り替える**（どちらで並べているかは凡例に出る）:
  ...
  - **作中の時刻** — 語と関係に書いた `when` の順（→ [関係と相関図](#関係と相関図)）。
  ...
  **並べるものも 2 つあり、軸の右の選択で切り替えます**（こちらも覚えます）:
  - **関係を並べる**（既定）— 1 行 1 関係。「誰と誰がいつ繋がるか」
  - **語を並べる（年表）** — 1 行 1 語。「何がいつ起きたか」。**軸に載るのは
  ...
  同じ事件は関係の本数だけ現れるので、事件を順に読みたいときは**語を並べる**ほうです。
