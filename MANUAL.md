...
⚙ の **AI** タブの「文体（口調）」に書いた指示が、下書きのプロンプトに入る。
「講談調で」「司馬遼太郎のような史伝の筆致で」「TRPG のルールブック風に」のように、
...
**口調を決めたら、顔も置ける。** 文体と同じ場所に `persona.png` という名前で画像を
置くと、**吹き出しと用語ページに丸く出る**。
...
- 使えるのは `.png` `.jpg` `.jpeg` `.webp` `.gif`。**SVG は出しません**
  （中身を検査せずに配る口なので、スクリプトを持てる形式は通さない）
...
- **一覧には出しません。** 顔は辞書に 1 枚なので、カードが並ぶと同じ絵が
  何十個も続くだけで何も区別できない
- ⚙ からは登録できません（置き場所の案内だけ出ます）。ファイルを置くだけなので、
  差し替えも消すのもエクスプローラで済みます
