...
上から順に **読むもの** → **その他の開き方** → **AI 設定** の 3 つ。
...
- **📁 フォルダ** — フォルダを選ぶ / お気に入り / 最近開いた / パス直接入力、
  そのフォルダのファイル一覧、**🔍 本文を横断検索**（畳んである。開閉は覚える）、
  **✨ フォルダから用語を抽出**
...
**📁 いま読んでいるものの AI 設定** では、そのフォルダだけに効く
[文体（口調）](#文体口調)と[語り手の顔](#語り手の顔)を差し替えられる。
**全体に効く指定は ⚙ の「AI」タブ**にあり、そちらには同じものが両方（全体と 📁）と、
**ファイルの置き場所のパス**も並んでいる（狭いのでこちらには出していない）。

本文の見せ方（テーマ・**各用語の最初の 1 回だけリンク**）は **⚙ の「表示」** にある。
...
左パネルの **🔍 本文を横断検索** を開いて語を入れ、**探す** と、開いているフォルダの
...
1. ソースを開く。次の 4 通り（→ [左パネルの構成](#左パネルの構成)）
...
左パネルの **📁 フォルダ** タブの **📁 フォルダを選ぶ…** で OS のフォルダ選択ダイアログが開く（ブラウザは
...
**そのフォルダだけの指定は、左パネルの「📁 いま読んでいるものの AI 設定」からも
書ける**（⚙ を開かずに済む）。中身は同じもので、⚙ 側には全体と 📁 の両方が並ぶ。
...
- **画面から選ぶ** — 左パネルの **📁 いま読んでいるものの AI 設定**（そのフォルダ用）
  または **⚙ の「AI」タブ**（全体と 📁 の両方）で、**選ぶ… / 差し替え…** を押して
  画像を選ぶ。🗑 で消せる
...
この 2 つは**ブラウザごと**に残る（データの保存先や更新の確認はアプリの設定
ファイル）。テーマは描画の前に当てる必要があるので、サーバへ聞きに行かず localStorage に
...
新しい版が出ていると、上部に小さく **`v0.9.0` が出ています** と出る（押すとリリース
...
`GlossPop-0.9.0` として展開する。展開したらそちらの `glosspop.exe` を起動すればよい。
