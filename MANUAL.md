...
先に ⚙ の **公開** タブで 2 つ決めます。**その場で効きます**（再起動は要りません）。

| 決めるもの | |
| --- | --- |
| 書き出し先のフォルダ | 例: `C:\Repos\mysite\htmlize`。**決めるまで書き出せません**（勝手にどこかへ書きません） |
| 公開先の URL | 例: `https://ユーザ名.github.io/リポジトリ/`。**書かないとカードの画像が出ません** |
...
| `GLOSSPOP_PUBLISH_DIR` | 無し | 公開ページの書き出し先。**設定ファイルより優先** |
