...
本文の見せ方（テーマ・**文字の大きさ**・**各用語の最初の 1 回だけリンク**）は
**⚙ の「表示」** にある。
...
**見せ方は 6 つある**（図の左上で切り替え。選んだものを覚える）。
...
- **時系列**（文書を開いているときだけ）— 上から順に、その文書を読み進めると
  ...
  **この見せ方だけ「いつ」が見える** —— 他の 3 つは辞書を平らに出すので、どの関係が
  ...
  本文を直せば次に開いたときに追いつく
- **地図**（座標を書いた語があるときだけ）— 絵を 1 枚敷いて、その上に語を点で置く。
  **この見せ方だけ「どこ」が見える**。線は、**両端がその絵に置かれている関係**だけ。
  絵が複数あるときは、見せ方の右の選択で切り替えられる（選んだものは覚える）。
  出し方は下記

#### 地図に置く

![桶狭間の戦いの地図。砦や田楽狭間が点で置かれ、迂回奇襲説の領域と正面攻撃説の経路が同じ絵の上に重なっている](docs/images/graph-map.png)
...
- **絵は画面から入れられる。** 地図の見せ方にして **🖼 絵** を押すと、置いてある絵の
...
- **頂点を足す / 消すのも同じ「置く」の中。** 線や領域の**線分の中点に出る小さな丸**を
  ...
  （それ以上消したいときは種別を変える）
- **種別は一覧で変えられる**（点 / 線 / 領域 / 地図から下ろす）。**置くと点になる**ので、
  **線や領域は「種別を変える → 頂点を足す」で作ります**。足りない点は隣に足され、
  落ちる点があるときは押す前に数が出ます
...
| 座標が絵の外（`x` が 0〜1 の外、`y` が負） | 注意 |

地図の 4 つは、どれも**画面には何も出ない**（書いたのに地図に現れないだけ）ので、
ここで言わないと気付く手段がない。逆に、**絵の名前だけ書いて座標がまだ無い**のは
「置き待ち」なので出さないし、縦長の絵で `y` が 1 を超えるのも正常
（座標は縦も横も**絵の幅**で割った比 → [地図に置く](#地図に置く)）。
...
**相関図の図の中だけは変わらない。** 箱の大きさもラベルの置き場所も図の側で
...
新しい版が出ていると、上部に小さく **`v0.11.1` が出ています** と出る（押すとリリース
...
`GlossPop-0.11.1` として展開する。展開したらそちらの `glosspopw.exe` を起動すればよい。
