...
- [使いかた](#使いかた)
  ...
  - [ビューアで登録する](#ビューアで登録する)
  - [まとめて登録する](#まとめて登録する)
  - [AI 執筆（テーマから本文と辞書を作る）](#ai-執筆テーマから本文と辞書を作る)
  - [フォルダを開く](#フォルダを開く)
    - [ファイルに画像を付ける](#ファイルに画像を付ける)
  ...
  - [冊子として書き出す](#冊子として書き出す)
  - [公開する（GitHub Pages など）](#公開するgithub-pages-など)
  - [辞書の点検](#辞書の点検)
  - [文書内のリンク](#文書内のリンク)
  - [URL を開く](#url-を開く)
...
**右パネルは目次と「この文書の用語」**。用語は**初出順**に並び、**2 回以上出てくる
語には回数が付く**（1 回のものには付かない —— 全部に「1」が並ぶと、多い語が
目立たなくなるため）。数えるのは本文のリンクと**同じ当たり方**なので、
インラインのコードの中は数え、コードブロックの中は数えない
（→ [自動リンクの規則](#自動リンクの規則)）。
**「各用語の最初の 1 回だけリンク」を入れていても回数は変わらない** ——
あれはリンクの見せ方の指定で、その語が何回出てくるかは別の話。
...
「辞書」「関係」を押しても、吹き出しから辞書ページへ飛んでも、**読んでいた本文は
...
別のものが出てくることはない。**この文書の関係**から戻る道（上部の「ビューア」）は、
その文書を直接指している。
...
表示中の文書の上にある **用語 ▾ → ✨ 用語を抽出** を押すと、その文書から辞書化する価値のある語を
...
2. **選んだ N 語の下書きを作る** で、選んだ語だけ下書きを作る
   （**1 語あたり数十秒**かかる。進捗が出て、途中で **中止** できる）
3. カテゴリと要約を見て、保存する語のチェックを残す。個別に直すなら **編集** で
   通常の登録ダイアログが開く
4. **チェックした N 語を保存** でまとめて登録される

下書きは**同時に 4 語ずつ**作られる（実測: 逐次 4 語 77.2 秒 → 同時 25.2 秒。
12 語なら 4 分ほどが 1 分半ほどになる）。同時数は選べない —— ブラウザが同じ相手へ
繋げる本数と、アプリが自分の窓を見張る仕組みから決まる値で、増やすと**下書きの
最中にサーバが自分で終わる**。順番は前後するので、進捗は「終わった数と、いま
走っている本数と経過秒」で出る（終わったときは合計の秒数も出る）。

#### 確認を省いて一息に入れる

フッタの **下書きを確認してから保存** を **下書きができ次第そのまま保存** にすると、
3 と 4 が消えて 1 回押すだけで辞書に入る。押すボタンは変わらず 1 つで、文言が
**選んだ N 語を下書きして保存する** に変わる。

保存したあとも各行の **編集** は残るので、そこから内容を直せる（この一覧が閉じるまでの
見直しの場になる）。閉じたあとは辞書の一覧から探す。

**この選択は覚えない。** 開くたびに「確認してから保存」に戻る —— 前回の選択が黙って
続くと、次に開いた人が 1 回押しただけで十数語が辞書に入ることになる。
...
丸ごと辞書化したいときは、**ファイルごとに 用語 ▾ → ✨ 用語を抽出 を回す**。以前は
...
**⋯ →「✨ AI 執筆…」**。ここまでの機能が「すでにある文書に辞書を付ける」のに対し、
こちらは**テーマから本文のほうを書く**。
...
| 項目 | 何を書くか |
| --- | --- |
| ジャンル | 歴史／思想・哲学／科学・技術／文学・芸術／社会・制度／自然・地理。**「おまかせ」なら AI が選ぶ** |
| テーマ、または仮定の論 | 「〇〇は△△だったのではないか」のような**仮定の形でよい**。空にすると AI が立てる |
| 語数 | 12 / 25 / 40。**本文の分量もここから決まる**（12 語 ≒ 1,800 字、40 語 ≒ 6,000 字）。次回に引き継がれる |

**文体（口調）の欄は無い。** ⚙ → AI で決めたもの（フォルダに置いた `.glosspop/style.md`
があればそちら）がそのまま効く。**抽出には効かないが、執筆には効く** —— 本文そのものを
書くので。

書けたらその場で直せる。気に入らなければ **「書き直す」** でもう一度。

#### 画面の見かた

左が本文、右がプレビュー（**登録済みの語はプレビューでもリンクになる**）。狭い画面では
縦に並ぶ。本文欄は最初から出ているので、**手持ちの文章を貼って、用語の書き出しだけ
使うこともできる**。

| ボタン | すること |
| --- | --- |
| ✍ 本文を書く / 書き直す | AI に本文を書かせる。すでに本文があれば書き直し |
| ✨ 用語を辞書に書き出す | **要る語を挙げ、一節を本文に入れ、辞書に登録するところまで** |
| 📋 本文をコピー | 本文の全文をクリップボードへ写す（ファイルは作らない） |
| 📄 本文を保存… | 開いているフォルダに `.md` として書く |
| ビューアで開く | この本文を画面に出す。**保存済みならファイルとして開く** |
| 閉じる | ダイアログを閉じる（書いたものは消えないが、保存はされない） |

#### ✨ 用語を辞書に書き出す

本文ができたら **「✨ 用語を辞書に書き出す」**。本文が**前提にしているのに名前が付いて
いない事柄**を、**その語を本文に登場させる一節ごと**挙げる。

チェックして押すと、**2 つのことが順に起きる**。

1. **一節が本文に入る**（指定された見出しの節の末尾。見つからなければ本文の末尾）
2. **その語が辞書に登録される**（語ごとに下書きして保存する。**同時に 4 語ずつ**進むので、
   語数ぶんの待ち時間がそのまま積み上がることはない）

**入る文章はそのまま画面に出る**ので、何が足されるのかを見てから決められる。
登録の結果は語ごとに行の中へ出る（`保存しました (📁 概念)` のように）。

**語だけでは足せない。** 本文に出てこない表記は登録してもリンクにならないので、
一節と対になっていないものは落とす。落としたものは「出せなかったもの」に理由つきで出る。

- 一節の中にその表記が無い
- **長い語に食われる**（`マルクス` は `マルクス主義` の中にしか出てこない、など）
- すでに辞書にある

#### 📄 本文を保存する

**辞書に書き出しても、本文はファイルにならない。** 別に保存する。

どこへ、どんな名前で書くかは**保存する前に画面に出る**
（`📁 <フォルダ> に「<名前>」として保存します`）。名前は本文の題（最初の `#` の行）から
付くので、変えたければその欄で直す。

**同じ名前のファイルがあるときは、先に断りが出る。** そのまま押しても保存されず、
「⚠ 同じ名前のファイルがすでにあります」と出たあと、**もう一度押したときだけ**
上書きになる。文書には控えを取る仕組みが無いので、消える前に見せる形にしてある。

保存すると、ボタンが **「ビューアで開く（保存したもの）」** に変わる。

| | 読書位置 | この文書の関係・時系列 | 横断検索 |
| --- | --- | --- | --- |
| ビューアで開く（保存前） | × | × | × |
| ビューアで開く（保存したもの） | ○ | ○ | ○ |

保存前は貼り付けと同じ扱いなので、**フォルダの中のファイルにしかできないことは効かない**。
ふつうに読み進めるなら、先に保存すること。

**GlossPop が利用者のファイルを書くのはここだけ。** ほかの画面はすべて読むだけで、
辞書は `.glosspop/` の中にしか書き込まない。**書いたものの片付けはこちらではしない**
ので、要らない下書きは手で消すこと。
...
ファイルには**絵を 1 枚**付けられる。本文の右上の **画像 ▾ → 🖼 画像を入れる…**
で選ぶ（上のバーの ⋯ ではない —— あちらは「読むもの」を選ぶ場所）。出るのは 2 か所:

- **本文の先頭**（カバー。高さは 320px までで、スクロールすれば流れる）
- **左のファイル一覧**（その行のサムネイル）

**地図とは別物。** 座標も関係も持たず、**一覧に出るだけ**の絵（地図は
[地図に置く](#地図に置く)）。

- **フォルダの中のファイルを開いているときだけ**置ける。貼り付け・ドロップ・URL には
  鍵になる相対パスが無い（[続きから読む](#続きから読む)が覚えないのと同じ理由）
- PNG / JPEG / WebP / GIF、**4 MB まで**。SVG は置けない
- 絵は辞書と同じ `.glosspop/files/` に、**相対パスを写して**入る
  （`章/一.md` なら `.glosspop/files/章/一.md.png`）
- **ファイル名を変えると絵は切れる。** 対応表を持たない代わりで、
  絵は残るので付け直せる
- 消すのは同じ **画像 ▾ → 🖼 画像を消す**（本文には触らない）
...
用語どうしの関係を書いておくと、辞書ページに一覧が出て、上部の **関係**（`/graph`）で
...
ものの図が見たいときは、ビューアの **関係 ▾ → 🕸 この文書の関係**（本文の右上）から開く。
...
- **相手がその文書に出てこない関係は落とす。** 足すとその文書に無い語が混ざって
  「この文書の関係」でなくなるため。**落とした本数は図の上に出る**（黙って欠けた図は
...
### 公開する（GitHub Pages など）

ビューアの上のバーの **⋯ → 🌐 公開する…** で、辞書を **1 枚のページ**にして、決めた
フォルダへ書き出せます。**[冊子](#冊子として書き出す)との違いは、貼ったときに出る絵
（メタ画像）が付くこと** —— X などに URL を貼ると、そのカードが出ます。

**何も決めなくても書き出せます。** ⚙ の **公開** タブで変えられますが、既定のままでも
手元で読める形にはなります（**その場で効きます**。再起動は要りません）。

| 決めるもの | 既定 | |
| --- | --- | --- |
| 書き出し先のフォルダ | `<開いているフォルダ>/.publish/` | **📁 選ぶ…** で選べます。`.` で始まるので、書き出したページは**ファイル一覧にも横断検索にも索引にも出ません**（出ると、次の公開でそれ自身を公開してしまいます） |
| 公開先の URL | 無し | **書かないとカードの画像が出ません。** 書き出し先が GitHub のリポジトリの中なら、**読み取った URL がボタンで出る**ので押すだけです |

**出力先はいつでも 1 つです。** 選べば既定は使われません（「下見はここ、本番はあちら」
という使い分けはしません —— 同じものが 2 か所にあると、どちらが最新か決まらなくなる
ため）。**切り替えても古いほうは消しません**ので、要らなくなったら手で消してください。

**URL の候補は読んだ値です。** 書き出し先のリポジトリの `.git/config` の remote と、
`CNAME`（独自ドメイン）から組み立てます。**押して確定するのはあなた**で、勝手には
入りません —— 外れた URL が入ると、**ページは出るのにカードだけ黙って出ない**ため。
`<ユーザ名>.github.io` というリポジトリ（ユーザ / 組織ページ）もそれと分かります。
**既定の場所のままでは候補を出しません** —— あそこは開いているフォルダの中なので、
遡ると**読んでいる作品のリポジトリ**に当たり、そこは配信元ではないからです。

書き出されるのは 3 つです。

| ファイル | |
| --- | --- |
| `<公開先>/<フォルダ名>/index.html` | カード ＋ 見出し ＋ 本文の一覧 ＋ 辞書ぜんぶ（冊子と同じ中身） |
| `<公開先>/<フォルダ名>/card.png` | メタ画像（1200×630） |
| `<公開先>/<フォルダ名>/docs/*.html` | **本文のページ。**自動リンクと吹き出しがそのまま効く |
| `<公開先>/<フォルダ名>/docs/*.png` など | その本文に付けた[画像](#ファイルに画像を付ける)（付けたものだけ） |
| `<公開先>/<フォルダ名>/assets/` | 見た目と吹き出しの仕掛け（**全ページで共有**） |
| `<公開先>/.nojekyll` | GitHub Pages の Jekyll を通さない印 |

- **フォルダ名は英数字にしてください**（ダイアログの「公開するフォルダ名」で変えられます。
  既定は開いているフォルダの名前）。日本語のままだと、**貼った先が URL を
  percent-encode せずに取りに来たときに 404 になり、カードが出ません** ——
  ブラウザは送る前に encode するので**打ち込むぶんには通ってしまい**、
  気づきにくいところです（日本語のままなら書き出すときに断りが出ます）
- **URL を書かないと画像だけ出ません。** メタ画像の URL は絶対でないと無視されるためで、
  **ページ自体は正しく出る**ぶん気づきにくいところです（書き出すときに断りが出ます）
- **見出しは辞書から機械で作ります** —— 同じ時刻に並んだ語 →
  いちばん多く繋がっている語 → [作中の時刻](#作中の時刻を書くwhen)の幅、の順に見て、
  **言えることが無ければ辞書の名前**になります
- **カードは押す前に見えます。** 見せている絵がそのまま書き出されます
- 入り切らない語は「**他 N 語**」として数で出ます（黙って落としません）
- **判明位置 (`reveal`) を書いた関係は数えません**（相関図の既定と同じ。
  カードのほうが人目に付くので、ここだけ緩めません）
- **本文のページにもカードが付きます。** 絵は**その文書に付けた
  [画像](#ファイルに画像を付ける)**で、ページの隣に同じ名前で置かれます
  （`docs/章.html` の隣が `docs/章.png`）。**辞書のカードでは代用しません** ——
  全部の文書が同じ絵になるうえ、貼った相手には「この文書の絵」に見えるためです。
  絵を付けていない文書は、見出しだけの小さいカードになります
- **1 枚が 2 MB を超えると、そのページのカードは出ません。** X のクローラの上限で、
  **ページ自体は読める**ぶん気づきにくいところです（超えたら書き出すときに
  断りが出ます。切りません）
- **本文のページは、受け取った人が GlossPop を持っていなくても読めます。**
  辞書はページに埋め込んであり、**通信もしません**（`file://` で開いても効きます）
- 埋め込む辞書は**その本文に出てくる語だけ**です。**リンクにならない語は入りません**
  （照合は本文と同じ `Linker`）
- **辞書ページへのリンクは張っていません。** 公開したサイトに辞書ページが無いので、
  Ctrl クリックで開くと 404 になるためです（吹き出しの中の語を辿る道は残っています）
- **判明位置 (`reveal`) を書いた関係は、本文のページでも伏せます**
- **手元のパスは出しません**（保存先・顔・画像の URL は落としてから埋め込みます）
- 本文が多いときは打ち切って、そう伝えます
- **貼ったときに出るのは画像だけ**です（説明文は出ず、見出しが小さく画像に重なります）。
  カードは**それ 1 枚で言い切れている**必要があります
- **公開してすぐは、カードが出ないことがあります。** 直後は枠すら出ず、しばらくして
  見出しと説明だけの小さいカードになる、という並びを見ています。ただし**何がそれを
  変えるのかは分かっていません**（時間なのか、タグの違いなのかを切り分けられていません）。
  **数分見て「タグが足りない」と判断しないでください** —— 直しても同じ URL では
  前の結果が返ってきます
- **X は一度読んだページの内容を覚えていて、読み直させる手段がありません。**
  カードを直したときは、**フォルダ名を変えて別の URL にする**のが確実です
- **commit と push はしません。書くだけです**
- 中身が同じなら画像の URL も変わりません（`?v=` に中身の印が入ります）。X はカードを
  **URL ごとに覚える**ので、変えたときだけ新しい URL になります

### 辞書の点検

`/doctor`（上部の **関係** → **⋯ → 🩺 点検**）で辞書全体を見る。参照を名前で書ける以上、
...
- `excludes` は**その表記では当てない**という指定（`aliases` の裏返し）。
  → [複合語に当たってしまうとき](#複合語に当たってしまうときexcludes)
...
### 複合語に当たってしまうとき（`excludes`）

**動詞から来た用語**（「読み」「控え」「間引き」）は、部分文字列マッチなので
**複合語や活用の中にも当たる** ——「読み込む」「読み直す」「読み上げ」の中の
「読み」にリンクが付く。用語名を選び直しても解けない（「読み」はその辞書の
正しい用語名で、たまたま生産的な語幹でもある、というだけ）。

`excludes` に**当てない表記**を並べる。`aliases` の裏返し。
...
- **語幹まで書けば足りる。**「読み直」と書けば 直す / 直し / 直さ が全部止まる
- 「同じ位置では長い表記が優先」に相乗りしているだけなので、新しい規則ではない
- **他の用語の表記のほうが強い。**「読み込み」という用語が別にあれば、そちらが勝つ
- **リンクにならない位置は「出てくる」とも数えない** ——
  [本文での使われ方](#本文での使われ方)・索引・相関図の `?doc=` も一緒に外れる
- 「最初の 1 回だけリンク」と併用しても、**除外した箇所が 1 回ぶんを奪わない**

自分の用語名を含まない表記（`読` だけ、無関係な語）は、どこにも当たらないので無視される。

**書かなくてよい辞書のほうが多い。**「うらなり」→「うらなり先生」のように、
続いていても正しい当たりであることは普通にある（`samples/坊っちゃん` が実例）。
機械が一律に切ると、そちらを壊す。
...
**写真を入れる代わりに、その語の図を描かせることもできる。** 用語ページの
**✏️ 図を作る…** から。**本文と要約に書かれていることだけ**を材料に、
図形と文字だけの図を 1 枚描いて、そのまま用語の画像にします。

聞かれるのは 2 つだけです。

| 何の図か | 描くもの |
| --- | --- |
| 配置・地形 | どこに何があるか。土地・建物・部屋・盤面を上から見た略図 |
| 構造・部分の名前 | ひとつのものを分解して、部分とその名前を示す。断面・部品・階層 |
| 流れ・手順 | 順番のあるものを、箱と矢印で並べる。分岐があれば分岐も |
| 対比・並べて比べる | 2 つ以上を同じ形式で並べ、違うところが目で分かるように |

もうひとつは**補足**（自由入力・任意）で、「北を上にする」「3 つの層に分ける」の
ように書けます。

- **何の図かは自分で選びます。** 選ばせずに「この語の図を」とだけ頼むと、AI は
  描きやすいものへ寄って、頼んだつもりのない図が出てきます
- **関係の図はここにはありません。** 関係・[作中の時刻](#作中の時刻を書くwhen)・
  地図は**辞書に書いた項目からそのまま描ける**ので、
  [辞書の見方](#見方は-6-つある)の側にあります（そちらは毎回同じ絵になります）。
  ここで描くのは、**本文には書いてあるのに、どの項目にも入っていないもの**です
- **入れるまで辞書は変わりません。** 描いた図はダイアログに出るので、見てから
  **この図を入れる**を押してください。気に入らなければもう一度描かせれば、
  違う絵が出てきます（**毎回同じにはなりません**）
- **描けないこともあります。** 材料にその図として描けることが書かれていなければ、
  AI は「描けない」と答えます —— **書かれていないことを補って描かせない**ため
  なので、そのときは本文を足してからもう一度どうぞ
- **入るのは PNG です**（ほかの画像とまったく同じ扱いで、一覧にも吹き出しにも
  地図にも出て、zip にも入ります）。**⬇ SVG で保存**を押すと、拡大できる形でも
  手元に落とせます —— **そちらは辞書には入りません**（外のエディタで直したいとき用）
- **AI が要ります**（→ [AI 下書きについて](#ai-下書きについて)）
...
新しい版が出ていると、上部に小さく **`v0.15.1` が出ています** と出る（押すとリリース
...
| `GLOSSPOP_PUBLISH_DIR` | `<開いているフォルダ>/.publish/` | 公開ページの書き出し先。**設定ファイルより優先** |
| `GLOSSPOP_PUBLISH_BASE_URL` | 無し | 公開先の URL（メタ画像を絶対 URL にするのに要る） |
...
`GlossPop-0.15.1` として展開する。展開したらそちらの `glosspopw.exe` を起動すればよい。
