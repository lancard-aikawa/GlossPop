...
**束ね方は 2 つ**（ツールバーの選択で切り替え、覚えます）。どちらも消しません ——
カテゴリは「どういう語か」で引く道、五十音は**辞書を通して引く**道です。
...
| 判明する位置 | 「第6章」など。**読者がいつ知るか**。相関図では既定で伏せられる |
...
相関図の**時系列**を「作中の時刻」で並べられます。

```yaml
relations:
- to: 田楽狭間
  label: 背後から急襲する
  when: 1560-05-19 永禄三年五月十九日
```
...
- **索引は保存しません。** 開くたびに読み直すので、外のエディタで書き換えたぶんも
  次に開けば追いつきます（→ [横断検索](#左パネルの構成)と同じ考え方）
...
辞書の **📖 冊子** で、辞書ぜんぶを**五十音順の 1 枚**にして書き出せます。
...
- 中身は**目次 → 五十音順の全項目**（読み・カテゴリ・別名・タグ・要約・本文・
  使用例・関係）。**書いてあるものだけ**が出ます
...
- **地図では点の代わりに出せます**（相関図の地図で「用語の画像」を入れる。絵ごとに
...
新しい版が出ていると、上部に小さく **`v0.13.1` が出ています** と出る（押すとリリース
...
`GlossPop-0.13.1` として展開する。展開したらそちらの `glosspopw.exe` を起動すればよい。
